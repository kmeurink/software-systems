...
public class Hotel {
	...
	public Hotel(String name) {
		...
		room102 = new PricedRoom(102, 50.50, 12);
	...
	/**
	 * Return the costs of the stay at the hotel.
	 */
	public ss.week3.bill.Bill getBill(String name, int nights, ss.week3.bill.Printer hotelPrinter) {
		if (!name.equals(null) && getRoom(name) instanceof PricedRoom) {
			hotelBill = new Bill(hotelPrinter);
			for (int i = 0; i < nights; i++) {
				hotelBill.addItem((PricedRoom) room102);
			}
			Safe hotelSafe = ((PricedRoom) room102).getSafe();
			hotelBill.addItem((PricedSafe) hotelSafe);
			return hotelBill;
		} else {
			return null;
		}
